...
namespace BookManagementPresentation.Controllers
{
    public class HomeController : Controller
    {
            ...
            string sql = "select * " +
                        ...
                        "where Books.ID in (select BookID " +
                                              "from(Select top(5) c.BookID, SUM(c.Quantity) as sumQuantity " +
                                                      ...
                                                      "where c.DDH_ID = ddh.DDH_ID and b.ID=c.BookID and b.isDelete=0 and ddh.OrderDate <= GETDATE() and ddh.OrderDate >= (GETDATE() - 7) and ddh.isPaid = 'true' " +
                                                      "group by BookID " +
        ...
        public ActionResult _TestPartialView(int? Page, string selectedValue)
        {
            ...
            if(selectedValue == "2")
            {
                ListSach = db.Books.Where(s => s.Quantity > 0 && s.isDelete == false && s.Quantity > 0).ToList().OrderBy(s => s.Name);
            }
            if (selectedValue == "3")
            {
                ListSach = db.Books.Where(s => s.Quantity > 0 && s.isDelete == false && s.Quantity > 0).ToList().OrderByDescending(s => s.Name);
            }
            if (selectedValue == "4")
            {
                ListSach = db.Books.Where(s => s.Quantity > 0 && s.isDelete == false && s.Quantity > 0).ToList().OrderBy(s => s.Price);
            }
            if (selectedValue == "5")
            {
                ListSach = db.Books.Where(s => s.Quantity > 0 && s.isDelete == false && s.Quantity > 0).ToList().OrderByDescending(s => s.Price);
            }
